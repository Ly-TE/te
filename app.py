...
app.config['TEMPLATES_AUTO_RELOAD'] = True  # 开发工具页面修改后立即重新加载模板，避免服务返回旧 HTML
...
PAGE_CONFIG = [
    ...
    {'url_path': '/AI_PRICE', 'template': 'AI_PRICE', 'title': 'AI用量看板', 'icon': 'fas fa-robot', 'show_menu': True, 'description': '查询 SpanAI Token 模型用量', 'color': 'linear-gradient(135deg, #667eea, #764ba2)'},
    ...
    {'url_path': '/referrer_simulator', 'template': 'referrer_simulator', 'title': '搜索来源模拟', 'icon': 'fas fa-magnifying-glass-location', 'show_menu': True, 'description': '模拟不同搜索引擎来源 document.referrer', 'color': 'linear-gradient(135deg, #f7971e, #ffd200)'},
    ...
    {'url_path': '/referrer_landing', 'template': 'referrer_landing', 'title': '搜索来源落地页', 'icon': 'fas fa-location-dot', 'show_menu': False},
]
...
HOME_TOOL_CARDS = [
    ...
    for item in PAGE_CONFIG
    if item.get('show_home_card', False)
]
...
def render_template_page(page_name):
    """通用页面渲染函数"""
    if page_name == 'referrer_landing':
        direct_target = (request.args.get('target') or '').strip()
        if direct_target.lower().startswith(('http://', 'https://')):
            return redirect(direct_target)
    ...
    if os.path.exists(template_path):
        ...
        try:
            # 开发工具箱页面经常直接改 templates 下的 HTML。
            # 在 debug=False 运行时也主动清理 Jinja 模板缓存，避免服务继续返回旧脚本。
...
@app.route('/referrer_source', methods=['GET'])
def referrer_source_page():
    """搜索来源模拟页：作为模拟搜索引擎来源，并从新页签打开用户输入的目标页面。"""
    engine = (request.args.get('engine') or 'other').strip()
    target = (request.args.get('target') or '').strip()
    search_referrer = (request.args.get('search_referrer') or '').strip()
    params = urllib.parse.urlencode({
        'target': target,
        'engine': engine,
        'search_referrer': search_referrer
    })
    # 打开用户输入的实际目标页。目标页读取到的真实 document.referrer 会是当前
    # /referrer_source?... 地址，其中携带 engine 和 search_referrer 参数，业务页可从
    # document.referrer 的查询参数中还原对应搜索引擎。
    landing_url = target or f"/referrer_landing.html?{params}"
    safe_engine = html.escape(engine)
    safe_target = html.escape(target)
    safe_search_referrer = html.escape(search_referrer)
    safe_landing_url = html.escape(landing_url, quote=True)
    ...
    safe_engine_name = html.escape(engine_names.get(engine, engine_names['other']))
    ...
    <head>
        ...
        <title>{safe_engine_name} 模拟搜索页</title>
        <link rel="stylesheet" href="/static/css/common.css">
        <style>
            body {{ background:#f5f7fa; color:#2c3e50; font-family:Microsoft YaHei,Arial,sans-serif; }}
            .search-card {{ max-width:860px; margin:56px auto; padding:30px; background:#fff; border-radius:14px; box-shadow:0 8px 28px rgba(0,0,0,.08); }}
            .search-logo {{ color:#667eea; font-size:30px; font-weight:800; margin-bottom:18px; }}
            .search-box {{ display:flex; gap:10px; margin:18px 0; }}
            .search-input {{ flex:1; padding:14px 16px; border:2px solid #dfe5ff; border-radius:10px; word-break:break-all; background:#f8f9ff; }}
            .result-item {{ margin-top:20px; padding:18px; border:1px solid #e8eaf2; border-radius:12px; }}
            .result-title {{ color:#1a0dab; font-size:18px; font-weight:700; }}
            .result-url {{ color:#188038; margin:6px 0; word-break:break-all; }}
            .btn {{ display:inline-block; margin-top:14px; padding:12px 18px; border-radius:9px; background:#667eea; color:#fff; text-decoration:none; font-weight:700; border:0; cursor:pointer; }}
            .tip {{ margin-top:16px; color:#795548; background:#fff8e1; border:1px solid #ffe0a3; border-radius:10px; padding:14px; line-height:1.7; }}
        </style>
        <script>
            function openLandingPage() {{
                var landingUrl = {json.dumps(landing_url, ensure_ascii=False)};
                // 在当前页签直接跳转，既能最可靠地携带 Referrer，又可以 100% 避免被浏览器弹窗拦截
                window.location.href = landingUrl;
            }}
        </script>
    </head>
    <body onload="openLandingPage()">
        <div class="search-card">
            <div class="search-logo">{safe_engine_name} 模拟搜索</div>
            <div class="search-box">
                <div class="search-input">{safe_target}</div>
                <button class="btn" type="button" onclick="openLandingPage()">打开结果</button>
            </div>
            <div class="result-item">
                <div class="result-title">目标页面 - 来源测试结果</div>
                <div class="result-url">{safe_target}</div>
                <p>这是模拟搜索引擎结果页。点击下方按钮会使用<strong>当前页签</strong>跳转到你输入的实际目标页面。目标页面读取到的真实 <code>document.referrer</code> 会是当前模拟搜索页 URL，并可从其中的 <code>engine</code> / <code>search_referrer</code> 参数还原对应搜索引擎。</p>
                <button class="btn" type="button" onclick="openLandingPage()">跳转到目标页面</button>
                <p id="popupTip" class="tip" style="display:none;">如果浏览器没有自动跳转，请点击上面的按钮手动跳转。</p>
            </div>
            <div class="tip">
                <p><strong>模拟搜索引擎：</strong>{safe_engine_name}（{safe_engine}）</p>
                <p style="word-break:break-all;"><strong>模拟搜索引擎 referrer 信息：</strong>{safe_search_referrer}</p>
                <p>目标页面地址：<a href="{safe_landing_url}" target="_blank">{safe_landing_url}</a></p>
            </div>
        </div>
    </body>
    </html>
...
def normalize_ai_price_usage_response(raw_data):
    """将 SpanAI 用量接口返回值标准化为前端页面需要的数据结构。"""
    payload = raw_data
    if isinstance(payload, dict):
        # 兼容常见外层结构：{data: [...]}, {data: {...}}, {result: ...}
        payload = payload.get('data', payload.get('result', payload.get('rows', payload)))
    ...
    for item in payload:
        ...
        total = item.get('total_quota_usd', item.get('total_quota', item.get('quota', item.get('amount', 0))))
        used = item.get('total_used_usd', item.get('used_quota_usd', item.get('used_quota', item.get('used', 0))))
        remain = item.get('total_remain_usd', item.get('remain_quota_usd', item.get('remain_quota', item.get('remain', 0))))
        models = item.get('models', item.get('model_usages', item.get('details', [])))
        ...
        for model in models:
            ...
            normalized_models.append({
                'model_name': model.get('model_name', model.get('model', model.get('name', '-'))),
                'request_count': model.get('request_count', model.get('requests', model.get('count', 0))),
                'prompt_tokens': model.get('prompt_tokens', model.get('input_tokens', 0)),
                'completion_tokens': model.get('completion_tokens', model.get('output_tokens', 0)),
                'used_quota_usd': model.get('used_quota_usd', model.get('used_quota', model.get('cost', 0)))
            })

        normalized_rows.append({
            'token_name': item.get('token_name', item.get('name', '')),
            'total_quota_usd': total,
            'total_used_usd': used,
            'total_remain_usd': remain,
            'models': normalized_models
        })
    return normalized_rows


@app.route('/api/ai-price-usage', methods=['GET'])
@apply_rate_limit(limit=1, window=60)
def ai_price_usage():
    """查询 AI 用量看板数据。"""
    token_name = (request.args.get('token_name') or '').strip()
    if not token_name:
        return jsonify({'success': False, 'error': 'token_name 参数不能为空', 'code': 'PARAM_ERROR'}), 400

    api_url = os.getenv('SPANAI_USAGE_API_URL', '').strip()
    if not api_url:
        return jsonify({
            'success': False,
            'error': 'AI 用量接口未配置，请在 .env 中配置 SPANAI_USAGE_API_URL',
            'code': 'SPANAI_API_NOT_CONFIGURED'
        }), 500

    try:
        headers = {'Accept': 'application/json'}
        api_token = os.getenv('SPANAI_API_TOKEN', '').strip()
        if api_token:
            headers['Authorization'] = f'Bearer {api_token}'
        new_api_user = os.getenv('SPANAI_NEW_API_USER', '').strip()
        if new_api_user:
        ...
        normalized_data = normalize_ai_price_usage_response(raw_data)

        return jsonify({
            'success': True,
            'data': normalized_data,
            'raw_success': raw_data.get('success') if isinstance(raw_data, dict) else None,
            'timestamp': datetime.now().isoformat()
        })
    except requests.exceptions.RequestException as e:
        logger.error(f"AI 用量接口请求失败: {e}", exc_info=True)
        return jsonify({'success': False, 'error': f'AI 用量接口请求失败: {str(e)}', 'code': 'SPANAI_REQUEST_FAILED'}), 502
    except ValueError as e:
        logger.error(f"AI 用量接口响应不是 JSON: {e}", exc_info=True)
        return jsonify({'success': False, 'error': 'AI 用量接口响应格式异常', 'code': 'SPANAI_RESPONSE_INVALID'}), 502
    except Exception as e:
        logger.error(f"查询 AI 用量失败: {e}", exc_info=True)
        return jsonify({'success': False, 'error': '查询 AI 用量失败', 'detail': str(e) if config.DEBUG else None, 'code': 'AI_PRICE_USAGE_FAILED'}), 500
...
if __name__ == '__main__':
    ...
    print("=" * 80)
    print("🌐 页面路由:")
    print("  🏠 首页: http://localhost/ 或 http://localhost/index.html")
    print("  🔓 SDK日志解密: http://localhost/sdk-decrypt 或 http://localhost/sdk_decrypt.html")
    print("  📊 频率限制统计: http://localhost/api/rate-limit-stats")
    print("=" * 80)
    print("⚠️ 频率限制配置:")
    ...
    # 启动 Flask 应用
    app.run(host='0.0.0.0', port=80, debug=config.DEBUG, threaded=True)
